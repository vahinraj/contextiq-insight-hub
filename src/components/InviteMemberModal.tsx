...
interface InviteMemberModalProps {
  isOpen: boolean;
  onClose: () => void;
  onInvite: (member: { name: string; method: string }) => void;
}
...
export const InviteMemberModal = ({ isOpen, onClose, onInvite }: InviteMemberModalProps) => {
  ...
  const handleEmailInvite = (e: React.FormEvent) => {
    ...
    onInvite({ name: email, method: "email" });
    setEmail("");
  ...
  const handleGithubInvite = (e: React.FormEvent) => {
    ...
    onInvite({ name: githubUsername, method: "github" });
    setGithubUsername("");
    toast.success("Invitation sent via GitHub!");
    onClose();
  };
      ...
      <DialogContent className="bg-[hsl(var(--surface))] border-[hsl(var(--stroke))] max-w-md">
        ...
        <Tabs defaultValue="email" className="mt-4">
          <TabsList className="grid w-full grid-cols-2 bg-[hsl(var(--surface-elevated))]">
            <TabsTrigger
              value="email"
              className="data-[state=active]:bg-[hsl(var(--accent-teal))] data-[state=active]:text-[hsl(var(--accent-teal-fg))]"
            >
